...
class RPC {
  ...
  start(configPath, callback, unsafeFlag=false) {
    ...
    async.waterfall([
      ...
      function(next) {
        ...
        self.checkReward(nodeId, next);
      },
      function(rewardResult, next) {
        if(rewardResult.wallet === '0x0000000000000000000000000000000000000000') {
          // self.shares.delete(nodeId);
          return next(new Error('Please STAKE first.'));
        }
        next(null);
      },
      function(next) {
   ...
   * 
   * {
        nodeId: '321321321',  // from paramete
        wallet: '0x2132132132132', //smart contract: statStorage. parameter: 0x + nodeId
        earnedGnx: 100, // query database table: paymentinfos where payStatus == 0, sum amount group by nodeId
        gasGnx: 5,
    }
   * @param {*} nodeId 
   * @param {*} callback 
  ...
  checkReward(nodeId, callback) {
    this._log(`attempting to checkReward for node id ${nodeId}`);
    if (!this.shares.has(nodeId)) {
      return callback(new Error(`node ${nodeId} not found or not running`));
    }

    let share = this.shares.get(nodeId);
    const bridge = share.config.bridges[0]
    this._log(`check reward from ${bridge.url}`);

    const reqUrl = `${bridge.url}/node/${nodeId}`
    axios.get(reqUrl).then(res => {
      callback(null, res.data)
    }).catch(err => {
      callback(new Error(`failed to chek reward from remote: ${err}`))
    })
  }
  ...
  getReward(nodeId, callback) {
    this._log(`getting reward ${nodeId}`);
    if (!this.shares.has(nodeId)) {
    ...
    let share = this.shares.get(nodeId);
    const bridge = share.config.bridges[0]
    this._log(`calling ${bridge.url}`);

    const reqUrl = `${bridge.url}/reward/${nodeId}`
    axios.get(reqUrl).then(res => {
      callback(null, (res.data))
    }).catch(err => {
      this._log('failed to getting reward ' + err)
      callback(new Error('failed to getting reward ' + err))
    })
  }
  /**

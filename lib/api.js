...
class RPC {
  ...
  start(configPath, callback, unsafeFlag=false) {
    ...
    async.waterfall([
      ...
      function(next) {
        ...
        if (self.running >= cpus().length && !unsafeFlag) {
  ...
  createWalletByV3json(json, password, name, callback) {
    if (!json) {
      return callback('missing v3json.');
    }
        
    try {
      var wm = walletManager.newWalletManager(utils.getWalletsFolderPath());
      var v3json = wm.importFromJson(json, password, name);
      return callback(null, {
        address: v3json.address
      });
    }
  ...
  getNodeToken(configPath, address, callback) {
    try {
      ...
      let nodeId = utils.getNodeID(config.networkPrivateKey);
      let hash = ethUtil.keccak256(`${nodeId}${address}`);
      let sig = ethUtil.ecsign(hash, privKey);
      callback(null, ethUtil.toRpcSig(sig.v, sig.r, sig.s) + '--' + nodeId);
    } catch(e) {
      callback(e);
    } 
  }
  ...
  get methods() {
    return {
      ...
      createWalletByV3json: this.createWalletByV3json.bind(this),

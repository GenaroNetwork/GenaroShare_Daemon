...
const genaroshare_logs = require('commander');
const fs = require('fs');
...
let address = null;
if (genaroshare_logs.remote) {
  address = genaroshare_logs.remote.split(':')[0];
  if (genaroshare_logs.remote.split(':').length > 1) {
    port = parseInt(genaroshare_logs.remote.split(':')[1], 10);
  }
}
...
utils.connectToDaemon(port, function(rpc, sock) {
  ...
  rpc.status((err, shares) => {
    ...
    for (let i = 0; i < shares.length; i++) {
      if (shares[i].id === genaroshare_logs.nodeid) {
    ...
    const fslogger = new FsLogger(logFileDir, genaroshare_logs.nodeid);
    ...
    setInterval(function() {
      if (currentFile !== fslogger._todaysFile()) {
        ...
        if (!utils.existsSync(fslogger._todaysFile())) {
          console.error(`\n  no logs to show for ${genaroshare_logs.nodeid}`);
          return sock.end();
        }

        logTail = new Tail(fslogger._todaysFile());
        let numLines = genaroshare_logs.lines
                     ? parseInt(genaroshare_logs.lines)
                     : 20;
